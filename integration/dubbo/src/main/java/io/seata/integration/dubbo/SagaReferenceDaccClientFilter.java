            ...
            if (DaccUtils.isDaccInvoke(serviceInterface, methodName)) {
                ...
                try {
                    Map<String, Object> ret = actionInterceptorHandler
                            .proceedDacc(arguments[0], xid);
                    BusinessActionContext actionContext = (BusinessActionContext) ret
                            .get(io.seata.common.Constants.SAGA_ACTION_CONTEXT);
                    RpcContext.getContext().setAttachment(RootContext.KEY_BRANCH_ID,
                            String.valueOf(actionContext.getBranchId()));
                } finally {
